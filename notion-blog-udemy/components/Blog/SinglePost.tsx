...
export const SinglePost = (post: postType) => {
    return (
        <>
            {post.isPagenationPage ? (
                <section className="lg:w-[49%] mb-8 border-2 border-sky-900 mx-auto rounded-md p-5 shadow-2xl hover:shadow-none hover:translate-y-1 transition-all duration-300">
                    <div className="flex items-center gap-3">
                        <Link href={`/posts/${post.slug}`}>
                            <h2 className="text-gray-950 text-2xl font-medium mb-2">
                                {post.title}
                            </h2>
                        </Link>
                        <div className="text-gray-950">{post.date}</div>
                    ...
                    <p className="text-gray-950">{post.description}</p>
                </section>
            ) : (
